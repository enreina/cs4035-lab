...
def computeprobabilityrecursion(modelIFST,sequence,index,state,DPdict):
  # Probability = P(final)
  (I,F,S,T) = modelIFST
  ...
  if tuple([state] + sequence[index:len(sequence)]) in DPdict:
...
def computeprobability(modelIFST,sequence,DPdict):
  (I,F,S,T) = modelIFST
  result = number(0.0)
...
def computeprobabilities(modelIFST,sett):
 (I,F,S,T) = modelIFST
 probs = []
 DPdict = dict()
...
def computeprobabilityrecursionreverse(modelIFST,sequence,index,state,DPdict):
  ...
  # Return the already hashed result
  if tuple([state] + sequence[0:index]) in DPdict:
...
def computeprobabilityreverse(modelIFST,sequence,DPdict):
  (I,F,S,T) = modelIFST
  result = number(0.0)
...
# Computes all probabilities in a given list of examples
def computeprobabilitiesreverse(modelIFST,sett):
 (I,F,S,T) = modelIFST
 probs = []
 DPdict = dict()
 for sequence in sett:
